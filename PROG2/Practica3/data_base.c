...
struct _data_base {
  ...
  btree* premiado_by_name;
  btree* premio_by_categoria;
};
...
data_base * new_data_base(const char *dirname)
{
    ...
    data->premiado_by_name = new_btree();
    data->premio_by_categoria = new_btree();
...
{   
    ...
    if(data_base_has_index(db) == TRUE){
        destroy_btree(db->premiado_by_name);
        destroy_btree(db->premio_by_categoria);
    }
...
list * find_all_premiados(data_base *bd){
    ...
    if(data_base_has_index(bd) == TRUE){
        btree_traverse(bd->premiado_by_name, lista);
        return lista;
    }
...
list * find_all_premios(data_base *bd){
    ...
    if(data_base_has_index(bd) == TRUE){
        btree_traverse(bd->premio_by_categoria, lista);
        return lista;
    }
...
list * find_premiado_by_nombre(data_base *bd, char *nombre){
    ...
    if(data_base_has_index(bd) == TRUE){
        galardonado = btree_find_element(bd->premiado_by_name, nombre);
        if(galardonado != NULL){
            list_insert(lista, galardonado);
        }
        return lista;
...
list * find_premio_by_categoria(data_base *bd, char *cat){
    ...
    lista = new_list();
     ...
     if(data_base_has_index(bd) == TRUE){
        galardon = btree_find_element(bd->premio_by_categoria, cat);
        if(galardon != NULL){
            list_insert(lista, galardon);
        }
        return lista;
...
void data_base_make_index(data_base *db){
    ...
    if(data_base_has_index(db) == TRUE){
        destroy_btree(db->premiado_by_name);
        db->premiado_by_name = new_btree();
        destroy_btree(db->premio_by_categoria);
        db->premio_by_categoria = new_btree();
    }
    
    for(i = 0; i < get_n_premiados(db->table_premiados); i++){
        galardonado = get_premiado_i(db->table_premiados, i);
        db->premiado_by_name = btree_insert_node(db->premiado_by_name, get_premiado_nombre(galardonado), galardonado);
    }
    
    for(i = 0; i < get_n_premios(db->table_premios); i++){
        galardon = get_premio_i(db->table_premios, i);
        db->premio_by_categoria = btree_insert_node(db->premio_by_categoria, get_premio_categoria(galardon), galardon);
    }
}
...
BOOL data_base_has_index(data_base *db){
    ...
    if(!(db->premiado_by_name) || !(db->premio_by_categoria))
        return FALSE;
